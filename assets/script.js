...
function renderNYTArticle(data) {
  ...
  articleContainerEl.setAttribute(
    "style",
    "display: flex; flex-direction: column; justify-content: space-between; height: 300px; align-items: center"
  );
  ...
  articleLinkEl.textContent =
    "View This Recent New York Times Article Related to - " +
...
function renderParkData(d) {
  ...
  var parkAddress =
    d.data[0].addresses[0].line1 +
    ", " +
    d.data[0].addresses[0].city +
    ", " +
    d.data[0].addresses[0].stateCode +
    " " +
    d.data[0].addresses[0].postalCode;
  ...
  parkContainerEl.setAttribute(
    "style",
    "display: flex; flex-direction: column; align-items: center; justify-content: center; width: 90%;"
  );
...
function fetchAPI(userSearch) {
  ...
  fetch(NYTrequest)
    ...
    .then(function (data) {
      ...
      var parkRequest =
        "https://developer.nps.gov/api/v1/parks?parkCode=&limit=1&q=" +
        userSearch +
        "&sort=&api_key=zcVsmk0xbipvcZpXfFUHrZuNkcfpYUIRiuic2fh1";
      fetch(parkRequest)
        .then(function (r) {
          return r.json();
        })
